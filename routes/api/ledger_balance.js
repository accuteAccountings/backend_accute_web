...
route.get('/total', auth, async (req, res) => {
  try {
    ...
    let t = 0;
    arr.map(e => {
      if (e.supplier_id === req.query.supplier) {
        t = parseInt(t) - parseInt(e.totalAmt);
      } else if (e.debit_acc_id === req.query.supplier) {
        t = parseInt(t) - (parseInt(e.amount) - parseInt(e.balance));
      } else if (e.customer_id === req.query.supplier) {
        t = parseInt(t) + parseInt(e.totalAmt);
      } else if (e.credit_acc_id === req.query.supplier) {
        t = parseInt(t) + parseInt(e.amount) - parseInt(e.balance);
      }
    });
...
route.get('/credited', auth, async (req, res) => {
  try {
    ...
    let t = 0;
    arr.map(e => {
      if (e.customer_id === req.query.supplier) {
        t = parseInt(t) + parseInt(e.totalAmt);
      } else if (e.credit_acc_id === req.query.supplier) {
        t = parseInt(t) + parseInt(e.amount) - parseInt(e.balance);
      }
