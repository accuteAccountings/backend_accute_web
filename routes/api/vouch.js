...
route.post("/", auth, async (req, res) => {
	...
	try {
		...
		costumer.Balance =
			parseFloat(costumer.Balance) + parseFloat(NewVouch.totalAmt);
		costumer.save();
				...
				[seq.Op.and]: [
					{
						[seq.Op.or]: [
							...
							{ customer_id: req.params.supplier_id },
						],
					},
					{
		...
		const recJO = await JoVouch.findAll({
			where: {
				[seq.Op.and]: [
					{
						[seq.Op.or]: [
							{ credit_acc_id: req.params.supplier_id },
							{ debit_acc_id: req.params.supplier_id },
						],
					},
					{
						bill_date: { [seq.Op.between]: [req.query.sdate, req.query.edate] },
